...
function drop(ev) {
  ...
  if (ev.target.classList.contains("borough")) {
...
function saveCurrentState() {
  ...
  localStorage.setItem('quiz2_state', JSON.stringify(currentState));
}

// Load saved state from localStorage
function loadSavedState() {
  // If we should start fresh, don't load saved state
  if (shouldClearState()) {
    return;
  }
  
  const savedState = localStorage.getItem('quiz2_state');
...
function checkAnswers() {
  ...
  .then(data => {
    ...
    console.log('Saved quiz2 results:', data);
  })
...
function clearQuizState() {
  localStorage.removeItem('quiz2_state');
}

// Function to determine if we need to clear localStorage
function shouldClearState() {
  // Check if we have existing saved state
  const hasExistingState = localStorage.getItem('quiz2_state') !== null;
  
  // Check referrer
  const referrer = document.referrer;
  
  // Quiz2 should ONLY preserve state on refresh or when coming from a later quiz (quiz3)
  ...
  // Only preserve state when coming from quiz3 (not quiz1)
  if (referrer && (referrer.includes('quiz/3') || referrer.includes('quiz3'))) {
    console.log("Coming from quiz3 - preserving state");
    return false;
  }
  
  // Clear state in all other cases (including coming from quiz1)
  console.log("Should clear state - fresh start for quiz2");
  return true;
}

document.addEventListener("DOMContentLoaded", function () {
  // Check if we need to clear any saved state
  if (shouldClearState()) {
  ...
  fetch("/static/data/quiz2.json?v=" + Date.now())
